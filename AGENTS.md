...
Byte Agent 是一个本地桌面编程助手，采用 **Tauri v2 + React** 桌面壳与 **Rust 本地 daemon** 分层的架构。当前是首个可运行的最小骨架：桌面壳通过 LF 分隔的 JSON-RPC over stdio 启动并驱动本地 daemon。

## 技术栈

- **后端 / daemon**：Rust workspace（`crates/*`）
  - `byte-protocol`：共享的 JSON-RPC 协议、JSONL 编解码、daemon 状态类型
  - `byte-daemon`：stdio JSONL JSON-RPC daemon 入口
...
# 运行 daemon 并手动调用 get_state
printf '{"jsonrpc":"2.0","id":1,"method":"get_state"}\n' | cargo run -q -p byte-daemon
...
- **桌面壳职责**：只负责启动 daemon、维护 stdio 传输、暴露 `get_daemon_state` 等 Tauri command。不实现业务逻辑或工具调用。
