...
A Compaction Entry reuses the summary card treatment (`message-summary`): a full-width card on the warm `warning-subtle` background with a `#fde68a` border and a 14px radius. The header row pairs the overline-styled "会话摘要" label with the entry timestamp and acts as an expand toggle when the entry carries a compacted message range; a small chevron on the right rotates 180° while expanded. The expanded footer is separated by a `#fde68a` top border and shows the compacted message range in the code typography so message ids stay scannable. The footer label and chevron use a deeper amber (`#92400e`) for readable contrast against the cream background. When no range is available the header is not interactive and the chevron is hidden.
