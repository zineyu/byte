	...
	const loadSession = useCallback(
		async (targetSessionId: string) => {
			try {
				...
				// A missing session is expected on first launch; genuine RPC failures
				// should surface in the connection state.
						...
						<div className="drawer-body">
							{groupedEvents.length > 0 ? (
								<ul className="event-list">
									{groupedEvents.map(({ event, count }, index) => (
										<li
											key={`${index}-${event.sequence}-${event.receivedAt.getTime()}`}
										>
...
function eventDetail(event: RuntimeEvent): string {
	switch (event.type) {
		...
		case "session_changed":
			return `会话 ${shortId(event.session_id)} ${event.action === "created" ? "已创建" : "已加载"}`;
	}
}
