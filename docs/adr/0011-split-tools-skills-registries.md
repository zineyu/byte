...
- **`activate_skill` 在 `byte-core` 实现并动态注册**
  ...
  - `ActivateSkillTool` 持有 `Arc<dyn SkillRegistry>`，调用 `SkillRegistry::activate` 后将结果追加到 `SessionRunner` 的内存 `active_skills` 列表，供后续 Run 的 `LlmContextBuilder` 注入上下文。（注入路径后由 ADR 0021 改为消息流注入，system prompt 不再包含已激活 Skill 内容。）
